const fs = require('fs');
let content = fs.readFileSync('src/pages/guru/RiwayatHafalan.tsx', 'utf8');

if (!content.includes('Edit2')) {
  content = content.replace("import { Loader2, Search, X } from 'lucide-react';", "import { Loader2, Search, X, Edit2, Trash2 } from 'lucide-react';");
}
...
content = content.replace(/<span className="text-\[10px\] font-black uppercase tracking-wider bg-emerald-50 text-emerald-600 px-2 py-1 rounded">{item.nilai}<\/span><\/td>\s*<\/tr>/g, '<span className="text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-600 px-2 py-1 rounded">{item.nilai}</span></td>\n' + actionsTd);
...
fs.writeFileSync('src/pages/guru/RiwayatHafalan.tsx', content);
