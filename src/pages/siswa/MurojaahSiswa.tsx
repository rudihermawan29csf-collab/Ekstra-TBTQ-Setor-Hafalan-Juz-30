...
export default function MurojaahSiswa() {
  const { user } = useAuth();
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const murojaahRecords = (setoranRes || []).filter((s: any) => s.siswa_id === user?.siswa_id && s.jenis === 'Murojaah');
      setData(murojaahRecords);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Riwayat Murojaah</h1>
      <p className="text-sm text-slate-500 italic mt-1">Daftar setoran pengulangan hafalan (Murojaah) Anda.</p>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center gap-3">
          ...
          <h2 className="font-bold text-slate-900">Murojaah Anda</h2>
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-100">
            ...
            <tbody className="bg-white divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan={4} className="p-8 text-center text-slate-500 font-medium">Belum ada riwayat murojaah.</td></tr>
